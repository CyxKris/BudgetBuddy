    ...
    public static MainStage getInstance() {
        ...
        if (stage == null) {
            synchronized (MainStage.class) { // Synchronize to ensure thread safety
                ...
                // * Race condition occurs when two or more threads access shared data and attempt to modify it at the same time. In the context of the getInstance() method, the race condition can occur if multiple threads simultaneously check for the existence of the mainStage instance and attempt to create a new instance. The synchronized block ensures that only one thread at a time can enter this critical section, preventing the race condition and ensuring that only one instance of MainStage is created.
                // *
                if (stage == null) {
